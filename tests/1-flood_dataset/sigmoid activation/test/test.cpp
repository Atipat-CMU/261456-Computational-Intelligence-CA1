...
int main(){
    ...
    for(int i = 0; i < k; i++){
        ...
        History history = network.fit(X_train_norm, y_train_norm, 1000, 10, 0.5);
        ...
        y_pred_train_norm.to_csv("text1.csv");
        y_pred_train.to_csv("text55.csv");
        y_train.to_csv("text2.csv");
        df.to_csv("dataset.csv");
